// app.js — static frontend that uses a Cloudflare Worker proxy for Device Flow
const PROXY = window.__PROXY_URL__ || '';
...
// Minimal repo placeholder renderer (you can replace this with your timeline rendering)
...
// Fetch repo metadata via GraphQL (uses your existing listOrgRepos query)
async function listOrgRepos(org) {
  ...
  while (hasNext) {
    ...
    if (!conn) break;
    conn.nodes.forEach(n => all.push({ name: n.name, updatedAt: n.updatedAt, openIssues: n.issues ? n.issues.totalCount : 0 }));
    hasNext = conn.pageInfo.hasNextPage; after = conn.pageInfo.endCursor;
  }
  return all;
}

// Fetch issues with your existing GraphQL query (getRepoIssuesWithSubIssues)
...
async function loadOrgData() {
  ...
  // fetch issues concurrently and render
  const concurrency = 4;
  ...
  async function runNext() {
    ...
    const p = getRepoIssuesWithSubIssues(ORG, repo.name)
      .then(issues => {
        ...
        // Replace placeholder with timeline rendering (or re-render in place)
        // For now update placeholder to show issue count
  ...
  const f = document.createElement('div'); f.className='footer'; f.textContent='Done.'; contentArea.appendChild(f);
}

// Device flow endpoints on your worker
async function startDeviceFlow() {
  setStatus('Requesting device code...');
  const r = await fetch(PROXY + '/device/code', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ scope: 'repo read:org' }) });
  const data = await r.json();
  if (data.error) { setStatus('Error: ' + (data.error_description || data.error)); return; }

  // show the code prominently
  setStatus('');
  contentArea.innerHTML = `<div class="intro">Enter this code on GitHub: <strong style="font-size:18px;">${data.user_code}</strong> — <a href="${data.verification_uri}" target="_blank">Open verification page</a></div>`;
  ...
  while (Date.now() < expiresAt) {
    ...
    const tokResp = await fetch(PROXY + '/device/token', {
      method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ device_code: data.device_code })
    });
...
oauthBtn.addEventListener('click', () => {
  startDeviceFlow().catch(e => { console.error(e); setStatus('Device flow failed: ' + e.message); });
});
